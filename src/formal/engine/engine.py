...
from formal.grammar import Grammar, compile_grammar
from formal.language import Language, get_language_info
...
class FormalEngine:
    __slots__ = ("model", "grammar", "_buffer", "_parser", "_tokenizer")

    model: Final[PreTrainedModel]
    ...
    def __init__(self, model_id: str, language: Language) -> None:
        ...
        tokenizer = cast(TokenizersBackend, tokenizer.backend)  # type: ignore

        model_vocab = tokenizer.get_vocab()
        language_info = get_language_info(language)
        grammar = compile_grammar(model_vocab, language_info)

        buffer = Buffer()

        lexer = Lexer(
            lex_tables=grammar.lex_tables,
            end_symbols=grammar.end_symbols,
            buffer=buffer,
        )
        ...
        scanner = (
            ...
            if Path(language_info.scanner_path).exists()
            else EMPTY_SCANNER
        )
    ...
    def evaluate(
        ...
        *,
        ...
        self._evaluate(input_id)

        for sequence_index in range(1, sequence_length):
            input_id = input_ids[sequence_index]
    ...
    def _evaluate(self, token_id: int) -> None:
        if token_id == self._tokenizer.eos_token_id:
            text = ""
            is_eos = True

        else:
            text = cast(str, self._tokenizer.decode(token_id))  # type: ignore
            is_eos = False
